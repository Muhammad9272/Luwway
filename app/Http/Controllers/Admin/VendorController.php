	        ...
	        $datas = User::where('is_vendor','=',2)->orWhere('is_vendor','=',1)->orderBy('id','desc')->get();
	         ...
	         return Datatables::of($datas)
                                ->addColumn('sales', function(User $data) {
                                  $sales=VendorOrder::where('user_id','=',$data->id)->where('status','=','completed')->sum('qty');
                                    return $sales;
                                })
                                ->addColumn('favorites', function(User $data) {
                                  $fav=FavoriteSeller::where('vendor_id',$data->id)->count();
                                    return $fav;
                                })
                ...
                '<option value="'. route('admin-vendor-st',['id1' => $data->id, 'id2' => 1]).'" '.$ns.'>Deactivated</option></select></div>';
	                            ...
	                            ->rawColumns(['sales','favorites','status','action'])
