...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        for($i = 1; $i <= 12; $i++) {
            ...
            $earnings .=  "'".Order::whereMonth('created_at', '=', date("m Y", strtotime('-'. $i .' months')))->sum('pay_amount')."',";
        }
        // dd($months);
...
// $today_users = User::whereDate('created_at', today())->count(); 4 4
// $month = Order::whereDate('created_at','>', today()->subMonths(1))->count();

// $month = Order::whereDate('created_at','>', today()->subMonths(4))->count();
...
// $month=null;
// $price=null;
// $orders=Order::where('status','=','completed')->select(DB::raw('sum(pay_amount) as sums'),DB::raw("MONTHNAME(created_at) as monthname"))
// ->whereYear('created_at', date('Y'))
// ->groupBy('monthname')
// ->get();
//  //dd($orders);
...
// }
  ...
  // dd($orders);
    ...
    // ->elementLabel('Earning')
    // ->labels($month)
    // ->values($price)
    // ->dimensions(1000,500)
    // ->responsive(false);

    // $demo=Charts::create('line', 'highcharts')
    // ->title('My nice chart')
