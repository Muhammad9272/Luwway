...
class CartController extends Controller
{
    ...
    public function cart()
    {
        ...
        $this->removeCoupons();
        if (!Session::has('cart')) {
            return view('front.cart')->with('success',"You don't have any product in cart.");
        }
        ...
        foreach ($cart->items as $product) {
            $shops[] = $product['item']['user_id'];
        }
        $shops = array_unique($shops);
        //dd($shops);

        return view('front.cart', compact('products','totalPrice','mainTotal','tx','shops')); 
    }
   ...
   public function addcart($id)
    ...
    public function coupon()
    {
        ...
        $ucat=Product::whereIn('id',$pid)->pluck('category_id')->toArray();      
        ...
        if(($coupon->t_check==1)?($from <= $today && $to >= $today):(1==1) )
        {   
            if($coupon->status == 1)
            {    
                $oldCart = Session::has('cart') ? Session::get('cart') : null;
                ...
                {  
                    ...
                    //********************** New Logic of coupons
                        if($coupon->apply_to==0){
                           $products=Product::whereIn('id',$pid);   
                        }
                        else if($coupon->apply_to==1){  //Logic Section for category

                            $id=[];
                            $apply_val=json_decode($coupon->apply_val);
                            
                            foreach($ucat as $unique){
                                  if(in_array($unique,$apply_val)){
                                    $id[]=$unique ; 
                                  }
                             }
                            $products=Product::whereIn('category_id',$id)->whereIn('id',$pid);   
                        }
                        else if($coupon->apply_to==2){
                            //Logic Section for Product
                            $id=[];
                            $apply_val=json_decode($coupon->apply_val);
                            foreach($pid as $unique){
                                  if(in_array($unique,$apply_val)){
                                    $id[]=$unique ; 
                                  }                                      
                             } 
                        ...
                        if($coupon->user_id!=0){
                        //if(count($users) == 1 && in_array($coupon->user_id,$users )){ //Main 
                        $products=$products->where('user_id',$coupon->user_id)->pluck('id')->toArray(); 
                        }
                        else{
                        ...
                        $price=0;
                        foreach ($cart->items as $prod) {
                                if(in_array($prod['item']['id'],$products))
                                $price+= $prod['price'];                                 
                        }   
                        if($price<=0){
                         return response()->json(0);
                        }    
                    //********************** New Logic of coupons ends
    ...
    public function code_image($value='')
    {
        # code...
    }
