...
class CheckoutController extends Controller
{
   ...
   //     if(isset($tax)){
   //       $tax_region=$tax->amount;
   //      return response()->json(array('tax_region'=>$tax_region,'states'=>$states)); 
   //     }
   //     else
   //     {
   //       $tax_region=$gs->tax;
   ...
   public function loadstates($id, $id2)
   {

      $checkout_country_id = null;
      $checkout_state_id = null;

      $gs = Generalsetting::findOrFail(1);
      if ($id2 == 1) {
         $checkout_country_id = $id;
         $tax = DB::table("taxes")->where("country_id", $id)->first();
         $states = DB::table("states")->where("country_id", $id)->get();
      } elseif ($id2 == 2) {

         $tax = DB::table("taxes")->where("state_id", $id)->first();
         ...
         $checkout_country_id = $country->id;
         $checkout_state_id = $id;
         if (!$tax) {
            $tax = DB::table("taxes")->where("country_id", $country->id)->first();
         }
      }
      ...
      if (Auth::guard('web')->check()) {
         ...
         $tax_amount=0;
         if ($gs->tax != 0) {
            $tax_amount = ($total / 100) * $gs->tax;
            $total = $total + $tax_amount;
         }
          if ($gs->currency_format == 0) {
            $tax_amount = $curr->sign . number_format($tax_amount,2);
         } else {
            $tax_amount = number_format($tax_amount,2) . $curr->sign;
         }
         ...
         if (!Session::has('coupon_total')) {
            ...
            Session::put('coupon_total', $ctt);
            //$total=str_replace("$", "",$total);    
         }
         $chk_buket = 1;
         if (Session::has('tax_region')) {
            Session::forget('tax_region');
         }
         Session::put('tax_region', $gs->tax);
         return view('includes.checkout.bucket', ['tax_amount'=>$tax_amount,'chk_buket' => $chk_buket, 'tax_region' => $gs->tax, 'states' => $states, 'checkout_country_id' => $checkout_country_id, 'checkout_state_id' => $checkout_state_id, 'products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'digital' => $dp, 'curr' => $curr, 'shipping_data' => $shipping_data, 'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);
      }
   }
...
public function checkout()
   {
      ...
      if (Auth::guard('web')->check()) {
         ...
         $userr = Auth::user();
         $states = null;
         if ($id = $userr->country_id) {
            $tax = DB::table("taxes")->where("country_id", $id)->first();
            $states = DB::table("states")->where("country_id", $id)->get();
         }
         if ($id = $userr->state_id) {
         ...
         return view('front.checkout', ['tax_amount'=>$tax_amount,'tax_region' => $gs->tax, 'states' => $states, 'products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'digital' => $dp, 'curr' => $curr, 'shipping_data' => $shipping_data, 'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);
      } else {
         $checked = 1;
         return view('front.checkout',compact('checked','curr'));
