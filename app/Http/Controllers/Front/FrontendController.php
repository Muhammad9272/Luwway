...
// -------------------------------- HOME PAGE SECTION ----------------------------------------
	...
	{   
        ...
        $sliders = DB::table('sliders')->where('is_video','!=',1)->get();
        $video_slider = DB::table('sliders')->where('is_video','=',1)->first();

        $top_banners = DB::table('banners')->where('type','=','TopSmall')->get();
        $mid_banners = DB::table('banners')->where('type','=','Large')->get();
        $bottom_banners = DB::table('banners')->where('type','=','BottomSmall')->get();

         $ps = DB::table('pagesettings')->find(1);
        
         $homeCards=DB::table('reviews')->orderBy('id','desc')->get();

         $subcategories=Subcategory::with('category')->where('is_featured',1)->where('status',1)->orderBy('id', 'desc')->get();
        
        $recent_viewed=null;
        if (Session::has('front.contact')){
             $products = session()->get('front.contact');      
             $products=array_unique($products);
             $recent_viewed=Product::whereIn('id', $products)->with('user')->take(10)->get(); 
        }


         $new_arrivals = Product::with('user')->where('top','=',1)->where('status','=',1)->orderBy('id','desc')->take(8)->get();

         $most_populars =  Product::with('user')->where('featured','=',1)->where('status','=',1)->orderBy('id','desc')->take(8)->get();

         $best_sellers = Product::with('user')->where('best','=',1)->where('status','=',1)->orderBy('id','desc')->take(8)->get();
         $instafeeds=$this->InstaFeed(); 
         //dd($instafeeds);
         
         return view('front.index',compact('ps','sliders','top_banners','mid_banners','bottom_banners','homeCards','new_arrivals','most_populars','best_sellers','subcategories','recent_viewed','video_slider','instafeeds'));
...
// CURRENCY SECTION
    ...
    public function currency11(Request $request)
    {
        $id=$request->curr_id;
        ...
        if (Session::has('coupon')) {
            ...
            Session::forget('coupon_percentage');
                ...
                if(Session::has('country_flag')){
          Session::forget('country_flag');
        }
        Session::put('country_flag',$country_flag);
        Session::put('currency', $id);
...
// -------------------------------- BLOG SECTION ----------------------------------------
	...
	public function blog(Request $request)
	{
        ...
        $bcats = BlogCategory::take(6)->get();
            if($request->ajax()){
                return view('front.pagination.blog',compact('blogs'));
            }
		return view('front.blog',compact('blogs','bcats'));
	}
    ...
    public function blogtags(Request $request, $slug)
    {
        ...
        return view('front.blog',compact('blogs','slug','bcats'));
    }
    ...
    public function blogsearch(Request $request)
    {
        ...
        return view('front.blog',compact('blogs','search','bcats'));
    }
    ...
    public function blogarchive(Request $request,$slug)
    {
        ...
        return view('front.blog',compact('blogs','date','bcats'));
    }
...
// -------------------------------- PAGE SECTION ----------------------------------------
     ...
     public function happyReviews()
    {
        ...
        $datas = $prods->where('status', 1)->has('ratings')->paginate(2);
       
        return view('front.happy-review',compact('datas'));
    }
...
// Maintenance Mode
    ...
    {
       $gs = Generalsetting::findOrFail(1);
       return view('front.comming-soon',compact('gs'));
    }
    ...
    public function loadstates($id)
    {
        ...
        return view('load.states',compact('country'));
    }
    ...
    public function partner()
    {
        $partners = DB::table('partners')->get();
        $ps =  DB::table('pagesettings')->where('id','=',1)->first();
        return view('front.partner',compact('ps','partners'));
    }



    public function partneremail(Request $request)
    {   
        ...
        $ps = DB::table('pagesettings')->where('id','=',1)->first();
        $subject = "Email From Of ".$request->name;
        $to = "partner@luwaay.com";
        $name = $request->name;
        $phone = $request->phone;
        $company=$request->company;
        $from = $request->email;
        $msg = "Name: ".$name."<br>Email: ".$from."<br>Phone: ".$request->country_code."&nbsp;".$request->phone."\nCompany: ".$request->company."\nMessage: ".$request->text;
        if($gs->is_smtp)
        {
        $data = [
            'to' => $to,
      ...
      {       
             ...
             // $RefreshTokens='https://graph.instagram.com/refresh_access_token?grant_type=ig_refresh_token&access_token=IGQVJVWGJkcjdTMXZAJUkhTRWxoYkVrc2dNRGlQU1VBM1RuZAGpYZAkZACaEEwdFBRT0lBbDRCaTByZA01FRjB3ZAG5vTlI2blV1dkNjdjR2V2JranFuSElKdkkwMjd4eGR2elA3dXhvX0FPLUduNUNURGFPbgZDZD';
             // $url=$RefreshTokens;
             $url = 'https://graph.instagram.com/me/media?fields=id,caption,media_url,permalink,thumbnail_url&access_token=IGQVJVWGJkcjdTMXZAJUkhTRWxoYkVrc2dNRGlQU1VBM1RuZAGpYZAkZACaEEwdFBRT0lBbDRCaTByZA01FRjB3ZAG5vTlI2blV1dkNjdjR2V2JranFuSElKdkkwMjd4eGR2elA3dXhvX0FPLUduNUNURGFPbgZDZD';
            $ch = curl_init();
