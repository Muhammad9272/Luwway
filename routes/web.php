...
Route::prefix('management001')->group(function() {
  ...
  Route::get('/users/removeall/rows', 'Admin\UserController@removeall')->name('admin-user-removeall');
  ...
  Route::get('/vendors', 'Admin\VendorController@index')->name('admin-vendor-index');
  Route::get('/vendor/bgimage', 'Admin\VendorController@color')->name('admin-vendor-color');
  ...
  Route::get('/general-settings/video-slider/{status}', 'Admin\GeneralSettingController@videoslider')->name('admin-gs-videoslider');
...
// Verification Section Ends
 ...
 //------------ ADMIN PRODUCT DISCUSSION SECTION ENDS ------------
  ...
  Route::get('/general-settings/announcement', 'Admin\GeneralSettingController@announcement')->name('admin-gs-announcement');
  ...
  Route::get('/general-settings/announcement/{status}', 'Admin\GeneralSettingController@isannouncement')->name('admin-gs-isannouncement');
  ...
  Route::get('/hcards', 'Admin\ReviewController@index')->name('admin-review-index');
...
//------------ ADMIN MENU PAGE SETTINGS SECTION ENDS ------------
  ...
  Route::group(['middleware'=>'permissions:payment_settings'],function(){
    ...
    Route::get('/tax/datatables', 'Admin\TaxController@datatables')->name('admin-tax-datatables');
    Route::get('/tax', 'Admin\TaxController@index')->name('admin-tax-index');
    Route::get('/tax/create', 'Admin\TaxController@create')->name('admin-tax-create');
    Route::post('/tax/create', 'Admin\TaxController@store')->name('admin-tax-store');
    Route::get('/tax/edit/{id}', 'Admin\TaxController@edit')->name('admin-tax-edit');
    Route::post('/tax/update/{id}', 'Admin\TaxController@update')->name('admin-tax-update');
    Route::get('/tax/show/{id}', 'Admin\TaxController@show')->name('admin-tax-show');
    Route::get('/tax/delete/{id}', 'Admin\TaxController@destroy')->name('admin-tax-delete');
...
Route::prefix('user')->group(function() {
  ...
  // User Reset
  Route::get('/account-setting', 'User\UserController@resetform')->name('user-reset');
  Route::post('account-setting/reset', 'User\UserController@reset')->name('user-reset-submit');

  Route::post('account-setting/reset/email', 'User\UserController@resetemail')->name('user-emailreset-submit');
  // User Reset End
...
// User Favorite Seller
  Route::get('/favorite/types', 'User\UserController@favoritestypes')->name('user-favorites-types');
  Route::get('/favorite/{type}', 'User\UserController@favoritesdetail')->name('user-favorites');

  Route::get('/favorites/{id1}/{id2}', 'User\UserController@favorite')->name('user-favorite');
  Route::get('/favorite/seller/{id}/delete', 'User\UserController@favdelete')->name('user-favorite-delete');
 
// User Favorite Seller Ends
    // user news starts
  Route::get('/news','User\UserController@news')->name('user-news');
  // user news Ends
  // User Logout
...
Route::post('the/maalik/2441139', 'Front\FrontendController@subscription');
...
Route::get('/under-maintenance', 'Front\FrontendController@maintenance')->name('front-maintenance');
  ...
  // User Register
  Route::get('/vendor/reg/', 'Vendor\VendorRegController@showRegisterForm')->name('vendor-register');
  Route::post('/vendor/register', 'Vendor\VendorRegController@register')->name('vendor-register-submit');

  Route::get('vendor/new-password/{token}', 'Vendor\VendorRegController@showVendorPasswordForm')->name('vendor.new.password');
  Route::post('vendor/new-password/{token}', 'Vendor\VendorRegController@VendorPasswordupdate')->name('vendor.new.password.update');
  Route::get('vendor/login', 'Vendor\VendorRegController@showVendorLoginForm')->name('vendor.login');
...
Route::prefix('vendor')->group(function() {
  ...
  Route::get('/reports/', 'Vendor\VendorController@report')->name('vendor-reports');
//------------ Store Policy ------------
Route::get('/policy', 'Vendor\VendorController@Policy')->name('vendor-get-policy');
Route::post('/policy/update', 'Vendor\VendorController@PolicyUpdate')->name('vendor-update-policy');
...
// Vendor Subscription
  ...
  Route::get('subscription/packages', 'Vendor\VendorController@package')->name('vendor-subpackage-index');
  Route::get('/subscription/{id}', 'Vendor\VendorController@vendorrequest')->name('vendor-subpackage-request');
  Route::post('/vendor-request', 'Vendor\VendorController@vendorrequestsub')->name('vendor-subpackage-request-submit');
  ...
  Route::get('/paypal/return/{token}/{subs_id}', 'Vendor\PaypalController@payreturn')->name('vendor.payment.return');
  Route::post('/paypal/notify', 'Vendor\PaypalController@notify')->name('vendor.payment.notify');
...
// Vendor Subscription Ends
  ...
  Route::get('/Schedule', 'Vendor\VendorController@Schedule')->name('vendor-store-Schedule');
  Route::post('/Schedule', 'Vendor\VendorController@ScheduleSave')->name('vendor-schedule.submit');
  Route::get('/Schedule/update', 'Vendor\VendorController@EditSchedule')->name('vendor-Schedule-edit');
  Route::post('/Schedule/update', 'Vendor\VendorController@UpdateSchedule')->name('vendor-Schedule-update');
...
// ************************************ FRONT SECTION **********************************************
  ...
  Route::post('/currency/', 'Front\FrontendController@currency11')->name('front.currency11');
  Route::get('/language/{id}', 'Front\FrontendController@language')->name('front.language');

  Route::get('/partner','Front\FrontendController@partner')->name('front.partner');
  Route::post('/partner','Front\FrontendController@partneremail')->name('front.partner.submit');
  Route::get('/comming-soon', 'Front\FrontendController@coming')->name('comming-soon');
  Route::get('/announcement','Front\FrontendController@announcement')->name('front.announcement');
  Route::get('/reviews','Front\FrontendController@happyReviews')->name('front.happyReviews');
  Route::get('/track','Front\FrontendController@trackOrder')->name('front.trackOrder');
  Route::get('/order/trackings/{id}', 'Front\FrontendController@trackload')->name('front-order-track-search');

  Route::get('/load/states/{id}/', 'Front\FrontendController@loadstates')->name('states-load'); //JSON REQUEST

  Route::post('/check-email','Front\FrontendController@userEmailCheck')->name('email_available.check');





  Route::get('vendor/type/{id?}','Front\VendorController@vendortype')->name('vendor.types');
  ...
  //vendor review
    Route::post('store/review/submit','Front\VendorController@vendorreviewsubmit')->name('front.vendor.review.submit');
  ...
  // CHECKOUT SECTION
  Route::get('checkout/load/states/{id1}/{id2}', 'Front\CheckoutController@loadstates')->name('checkout-states-load');

  //Route::get('checkout/states/tax', 'Front\CheckoutController@statestax')->name('checkout-states-tax'); 
  //JSON REQUEST
  ...
  Route::get('/checkout/payment/return/{custom}/{item_number}/{token}', 'Front\PaymentController@payreturn')->name('payment.return');
  ...
  // Route::get('/stores/','Front\VendorController@index')->name('front.stores');


  Route::get('stores/','Front\VendorController@index')->name('front.stores');
  Route::get('store/{store_name}/{category?}/{subcategory?}/{childcategory?}','Front\VendorController@show')->name('front.vendor');
